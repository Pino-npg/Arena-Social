...
function chooseMode(mode){
  ...
  walletBtn.disabled=true; demoBtn.disabled=true;
  ws.send(JSON.stringify({ type:'start', mode:mode, character:currentPlayer.character }));
  playBattleMusic();
}
...
ws.onmessage=(event)=>{
  ...
  if(msg.type==="ready") currentPlayer.index=msg.playerIndex;
  ...
  if(msg.type==="init"){
    ...
    updatePlayersUI(); startBtn.disabled=false;
  }
...
function updatePlayersUI(){
  players.forEach((p,i)=>{
    ...
    if(i===0) player1Img.src=imgEl.src; else player2Img.src=imgEl.src;
  });
}
...
function updateCharacterImage(player,index){
  let hp=player.hp; let charName=player.character; let state='';
  if(hp<=0) state='0'; else if(hp<=5) state='5'; else if(hp<=10) state='10'; else if(hp<=15) state='15';
  const src=state?`img/${charName}${state}.png`:`img/${charName}.png`;
  document.querySelector(`#player${index+1} img.character`).src=src;
  if(index===0) player1Img.src=src; else player2Img.src=src;
}
