...
walletBtn.onclick = async () => {
  await connectWalletAndFetchNFTs();
  chooseMode('wallet');
};
demoBtn.onclick = () => chooseMode('demo');

// --- FUNZIONE CHOOSE MODE ---
...
async function connectWalletAndFetchNFTs(){
  ...
  try{
    ...
    const balance = await pinoContract.balanceOf(address);
    for(let i=0;i<balance;i++){
      const tokenId = await pinoContract.tokenOfOwnerByIndex(address,i);
      if(pinoRank[tokenId]){
    ...
    const yumiBalance = await yumiContract.balanceOf(address);
    if(yumiBalance > 0){
      bonus.hp += 2; // tutti i YUMI danno +2 HP
    }

    currentPlayer.bonusHP = bonus.hp;
    currentPlayer.bonusDamage = bonus.damage;
    currentPlayer.bonusInitiative = bonus.initiative;

    console.log("🎁 Bonus dal wallet:", bonus);
